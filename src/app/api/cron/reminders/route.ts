...
function parseISTDate(dateStr: string, timeStr: string): Date | null {
    try {
        // Extracts "06:00" or "06:00 AM" from "06:00 AM - 07:00 AM"
        const startTimeStr = timeStr.split(' - ')[0].trim();
        const [time, period] = startTimeStr.split(' ');
        let [hours, minutes] = time.split(':').map(Number);
        
        // Handle malformed strings gently
        if (isNaN(hours) || isNaN(minutes)) return null;
        
        // If period exists (AM/PM), calculate 24-hr. Else assume it's already 24-hr
        if (period) {
        ...
        const isoDateStr = new Date(dateStr).toISOString().split('T')[0];
    ...
    try {
        ...
        for (const doc of bookingsSnapshot.docs) {
            try {
                ...
                if (!matchStartTime) continue; // Skip if the date was malformed
                ...
                console.error(`Failed sorting booking document ${doc.id}:`, innerError);
            }
        }
