...
import { MapPin, ArrowRight, Search, SlidersHorizontal, ChevronDown, X, Star } from 'lucide-react';
...
export default function TurfsPage() {
    ...
    // Extract unique cities for the dropdown, including all base cities from db
            ...
            <div className="container mx-auto px-4 sm:px-6 pt-28 sm:pt-36 pb-12 relative z-10">
                <div className="mb-10 sm:mb-14 text-center sm:text-left animate-fade-up">
                    ...
                    <p className="text-gray-400 max-w-2xl text-base sm:text-lg mx-auto sm:mx-0 font-medium">
                        Browse our curated selection of top-tier cricket turfs. Find the perfect pitch, book instantly, and elevate your game.
                    </p>
                </div>

                {/* Filter Bar */}
                <GlassCard className="mb-8 sm:mb-10 p-2 border-white/10 animate-fade-up" style={{ animationDelay: '0.2s' }}>
                    <div className="flex flex-col lg:flex-row gap-3">
                        {/* Search */}
                        <div className="relative flex-1">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none group-focus-within:text-[var(--turf-green)] transition-colors" />
                            <input
                                type="text"
                        ...
                        <div className="flex items-center gap-3 flex-wrap bg-white/[0.02] p-2.5 rounded-xl border border-white/5 w-fit">
                            ...
                            {selectedLocation !== 'all' && (
                                <span className="inline-flex items-center gap-1.5 text-xs bg-[var(--turf-green)]/10 text-[var(--turf-green)] px-3 py-1.5 rounded-full border border-[var(--turf-green)]/20 font-medium tracking-wide">
                                    ...
                                    <button onClick={() => setSelectedLocation('all')} className="ml-1 hover:text-white hover:bg-[var(--turf-green)]/20 rounded-full p-0.5 transition-colors"><X size={12} /></button>
                                </span>
                            )}
                            ...
                            {sortBy !== 'default' && (
                                <span className="inline-flex items-center gap-1.5 text-xs bg-blue-500/10 text-blue-400 px-3 py-1.5 rounded-full border border-blue-500/20 font-medium tracking-wide">
                                    ...
                                    <button onClick={() => setSortBy('default')} className="ml-1 hover:text-white hover:bg-blue-500/20 rounded-full p-0.5 transition-colors"><X size={12} /></button>
                                </span>
                            )}
                ...
                {loading ? (
                    ...
                    <GlassCard className="text-center py-20 px-4 border-white/5 bg-white/[0.02] animate-fade-up flex flex-col items-center justify-center max-w-2xl mx-auto mt-10" style={{ animationDelay: '0.4s' }}>
                        <div className="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center mb-6">
                            <Search className="w-10 h-10 text-gray-500" />
                        </div>
                        <h3 className="text-2xl lg:text-3xl text-white font-bold mb-3 tracking-tight">No courts found</h3>
                        <p className="text-gray-400 text-lg mb-8 max-w-md mx-auto">We couldn't find any courts matching your current filters. Try adjusting your search.</p>
                        {hasActiveFilters && (
                            <button 
                        ...
                        {filteredTurfs.map((turf, index) => (
                            ...
                            >
                                <Link href={`/turfs/${turf.id}`} className="block relative h-56 sm:h-64 w-full overflow-hidden">
                                    ...
                                    {/* Bottom Title Overlay (Mobile mainly, or redundant if we have title below, but let's keep it below for cleaner look) */}
                                </Link>
                                    ...
                                    <div className="flex items-center justify-between pt-5 border-t border-white/10 mt-auto group-hover:border-[var(--turf-green)]/30 transition-colors">
                                        <div className="flex flex-col">
                                            ...
                                            <div className="flex items-end gap-1">
                                                <span className="text-[var(--turf-green)] text-xl font-black leading-none group-hover:animate-pulse-glow">₹{turf.pricePerHour}</span>
